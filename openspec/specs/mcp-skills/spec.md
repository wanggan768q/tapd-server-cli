# mcp-skills Specification

## Purpose
MCP skill 文件的内容规范与 install-skills 子命令的行为规范。定义 10 个 skill 的结构要求、双语触发词约束、默认查询行为（owner=$ME）、各 skill 工作流约束，以及 install-skills / uninstall-skills 子命令的安装、升级冲突处理、跨客户端落地与卸载行为。
...
涉及列表查询的 skill（首先是 `tapd-my-work` 和 `tapd-overview`）MUST 在工作流中显式说明：**当用户未指定查询对象时（包括泛查询如"查 bug"、"看需求"，不仅限于含"我的"的措辞），默认使用 `~/.tapd/cache.json` 中的 `identity.tapdUserName` 作为 `current_owner` filter**；当用户明确指定他人（如"张三的需求"）或明确说"全部/所有人"时才覆盖或移除该过滤器。

#### Scenario: 泛查询默认查我

- **WHEN** 模型加载 `tapd-my-work` 或 `tapd-overview` 后处理"查 bug"/"看下需求"等不含人名的查询
- **THEN** skill 文本 MUST 引导模型把 `current_owner` 设为 `$ME`（来自 cache.json）
- **AND** MUST 不主动询问"是查谁"

#### Scenario: 含"我的"的查询默认查我
...
- **WHEN** 模型处理"本期所有人的 bug"/"全部 bug 不过滤人"
- **THEN** skill 文本 MUST 引导模型移除 `current_owner` 过滤器
...
- **THEN** CLI MUST 退出码 2
- **AND** stderr MUST 提示"管理者 skill 上线后此命令启用"
